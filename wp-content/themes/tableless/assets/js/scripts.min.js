...
var tableless=(function(){'use strict';function init(){showFeaturedPosts();prettyPrintHighlight();openCloseSearch();searchAjax();showMenu();}
...
function searchAjax(){$('#searchsubmit').on('click',function(e){e.preventDefault();var searchTerm=$('#s').val();$.ajax({url:"http://tableless.com.br/?s="+searchTerm,type:'get',success:function(data){var dataContent=$('<div class="tb-data-box">').html(data).find('.tb-search-content').html();$('.tb-search-results-list').html(dataContent);}});});}
...
function showMenu(){$('.tb-sandwich').on('click',function(){$('.tb-menu').toggleClass('tb-is-active');});}
